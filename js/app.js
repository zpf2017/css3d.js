...
$(function () {
    ...
    function a() {
        ...
        e > t ? ($("#three").css({
            ...
            app.stage.size(e, t).update());
    }

        app.stage = new C3D.Stage;
        app.stage.size(window.innerWidth, window.innerHeight).material({color: "#000"}).update();
        app.logo = C3D.create({
            type: "sprite",
            name: "logo",
            position: [0, 0, 0],
        ...
        app.stage.addChild(app.logo);
        app.page1 = C3D.create({
            type: "sprite",
            name: "page1",
            position: [0, 0, 0],
            children: [{
        ...
        app.stage.addChild(app.page1),
        //app.stage.addChild(app.page2),
        //app.stage.addChild(app.page3),
        $("#three").addClass("page1");
        document.getElementById('three').appendChild(app.stage.el)
        //$("#three").prepend(app.stage);
        app.initOrientation(),
        app.timer = setInterval(a, 33)
}),
    app.random = function (e, t) {
        return e + Math.floor(Math.random() * (t - e))
    },
    app.shockPos = {x: 0, y: 0},
    app.orient = {alpha: 0, beta: 0, gamma: 0},
    app.initOrientation = function () {
        var e = this;
    ...
    app.aimPos = {x: 0, y: 0, z: 0, rx: 0, ry: 0, rz: 0},
    app.update = function (e) {
        this.stage.camera.rotationY += .3 * (e - this.stage.camera.rotationY),
         this.stage.camera.rotationZ += .3 * (this.aimPos.rz + .2 * e - this.stage.camera.rotationZ),
            this.aimPos.x += .3 * (4 * e - this.aimPos.x),
            this.stage.camera.x = this.aimPos.x + this.shockPos.x, this.stage.camera.y = this.aimPos.y + this.shockPos.y, this.stage.updateT()
    },
